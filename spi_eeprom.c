/*SPI EEPROM Slave Device Driver*/
...
static char instruction_buffer[4]; // INSTRUCTION + ADDRESS
...
/*Address where to write or read*/
static int location;
...
static ssize_t eeprom_write(struct file *file, const char __user *buf, size_t len, loff_t *off)
{
  ...
  while (rem_bytes != 0)
  {
    ...
    printk("Write Enable Latch - WEL: %d\n", ret);
    if (writing_location % PG_SIZE != 0)
    {
      wr_len = ((writing_location / PG_SIZE) + 1) * PG_SIZE - writing_location;
      if (wr_len > rem_bytes)
      {
        wr_len = rem_bytes;
      }
    }
    else if (rem_bytes > 0 && rem_bytes < PG_SIZE)
    {
      wr_len = rem_bytes;
    }
    else
    ...
    page_buffer[0] = WR;
    page_buffer[1] = (writing_location >> 16) & 0xff;
    page_buffer[2] = (writing_location >> 8) & 0xf;
    page_buffer[3] = (writing_location & 0xff);
    ...
    printk("Writing: %s\n", (page_buffer + 4));
    transfer[0].tx_buf = page_buffer;
    transfer[0].len = wr_len + 4;
    transfer[0].speed_hz = SPI_SPEED_HZ;
    ...
    writing_location = writing_location + wr_len;
  ...
  // Disabling Write Enable latch
  instruction_buffer[0] = WRDI;
  ret = spi_write(spi_device, instruction_buffer, 1);
  if (ret < 0)
  {
    pr_err("failed to disable write enable latch\n");
    return -1;
  }
  ret = read_status_register(GET_WEL);
  printk("Write Enable Latch - WEL: %d\n", ret);
  memset(instruction_buffer, 0, 4);
  return len;
}
...
{
  int ret = 0;
  int read_location = location;

  printk("Reading from location: %d\n", read_location);
  instruction_buffer[0] = RD;
  instruction_buffer[1] = (read_location >> 16) & 0xff;
  instruction_buffer[2] = (read_location >> 8) & 0xff;
  instruction_buffer[3] = (read_location & 0xff);

  printk("instruction_buffer[0]: %02x\n", instruction_buffer[0]);
  printk("instruction_buffer[1]: %02x\n", instruction_buffer[1]);
  printk("instruction_buffer[2]: %02x\n", instruction_buffer[2]);
  printk("instruction_buffer[3]: %02x\n", instruction_buffer[3]);

  transfer[0].tx_buf = instruction_buffer;
  transfer[0].len = 4;
  transfer[0].speed_hz = SPI_SPEED_HZ;
  // data to receive
  transfer[1].rx_buf = eeprom_buffer;
  transfer[1].len = len;
  transfer[1].speed_hz = SPI_SPEED_HZ;
...
static long eeprom_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
  ...
  case PE:
  {
    ...
    ret = read_status_register(GET_WEL);
    printk("Write Enable Latch - WEL: %d\n", ret);
    instruction_buffer[0] = PAGE_ERASE;
    instruction_buffer[1] = (address >> 16) & 0xff;
    instruction_buffer[2] = (address >> 8) & 0xff;
    instruction_buffer[3] = (address & 0xff);

    ret = spi_write(spi_device, instruction_buffer, 4);
    if (ret < 0)
    {
  ...
  case SE:
  {
    ...
    ret = read_status_register(GET_WEL);
    printk("Write Enable Latch - WEL: %d\n", ret);
    instruction_buffer[0] = SECTOR_ERASE;
    instruction_buffer[1] = (address >> 16) & 0xff;
    instruction_buffer[2] = (address >> 8) & 0xff;
    instruction_buffer[3] = (address & 0xff);
    ret = spi_write(spi_device, instruction_buffer, 4);
